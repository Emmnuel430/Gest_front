...
const Sidebar = ({ user }) => { // Définition du composant Sidebar qui prend un utilisateur en prop.

  return (
    <div className="sidebar b-bar d-flex pb-3">
      <div className="navbar bg-light navbar-light">
        ...
        <a href="#" className="navbar-brand mx-4 mb-3 d-flex align-items-end">
          <img
              ...
              height="40"
            />
          <h3 className="m-0" style={{color: "#ed3731"}}> {/* Affichage du titre "Gest v1" */}
        ...
        <div className="d-flex align-items-center ms-4 mb-4">
          ...
          {user && ( // Vérifie si l'utilisateur est défini (authentifié).
            <div className="ms-3">
              <h6 className="mb-0">
                {user.prenom} <strong>{user.nom}</strong> {/* Affiche le prénom et le nom de l'utilisateur */}
              </h6>
              <span>{user.role ? "Admin" : "Staff"}</span> {/* Affiche le rôle de l'utilisateur (Admin ou Staff) */}
            </div>
          )}
        </div>

        {/* Inclure les liens de navigation */}
        <SidebarLinks user={user} /> {/* Affichage des liens en fonction de l'utilisateur */}
      </div>
    </div>
  );
};
